...
export const calculatePlayerRolesFromMatrix = ({
  battingMatrix,
  bowlerMatrix,
  battingList,
  bowlingList,
  ...
  const onGroundPlayer = [...batsmen, ...bowlers].filter(
    (player) => player.role !== "RR"
  );
...
export const calCulatedRankPlayers = (playersData = []) => {
  ...
  return sortedData.map((player, index) => {
    const goal_pts =
      index === 0 ? 0 : sortedData[index - 1].total_pts - player.total_pts + 1;
    return {
      ...player,
      goal_pts,
    };
...
export const showCurretOverBatsmenAndBowler = ({
  battingMatrix,
  bowlerMatrix,
  battingList,
  bowlingList,
